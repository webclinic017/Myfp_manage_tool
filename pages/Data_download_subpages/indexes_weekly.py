...
def body():
    # 指数list
    stock_index_list = {'上证指数': '000001.SH', '沪深300': '000300.SH', '中证500': '000905.SH', '中证1000': '000852.SH',
    ...
    with st.spinner('正在处理中...'):
        ...
        st.subheader('日期设置')
        this_week = st.selectbox('本周日期', options=stock_index_update.index,
                                 index=stock_index_update.shape[0] - 1)
        last_week = st.selectbox('上周日期', options=stock_index_update.index,
                                 index=stock_index_update.shape[0] - 6)
        week_before = st.selectbox('两周前日期', options=stock_index_update.index,
                                   index=stock_index_update.shape[0] - 11)

        result = st.button('显示股票市场指数结果')
        ...
        # 商品指数
        result_commodity = st.button('显示商品市场指数结果')
        if result_commodity:
            st.write('商品指数')

            c_this_week_return = commodity_index_update.loc[this_week, :] / commodity_index_update.loc[last_week,
                                                                            :] - 1
            c_last_week_return = commodity_index_update.loc[last_week, :] / commodity_index_update.loc[week_before,
                                                                            :] - 1
